          ...
          <tbody>
            {rows.map((b: any) => (
              <tr key={b.id} className="border-t">
                <td className="py-3 font-medium">{`B${String(b.id).padStart(4, "0")}`}</td>
                <td className="py-3">{b.contact_name}</td>
                <td className="py-3">{b.services?.title || b.other_therapy || "—"}</td>
                <td className="py-3">
                ...
                <td className="py-3 flex gap-2">
                  <button onClick={() => setEditRow(b)} className="p-2 rounded-lg hover:bg-purple-50">
                    <Pencil size={16} />
                  </button>
                  <button onClick={() => del(b.id)} className="p-2 rounded-lg hover:bg-red-50 text-red-600">
                    <Trash2 size={16} />
                  </button>
                </td>
              </tr>
            ...
            {rows.length === 0 && (
              <tr><td colSpan={7} className="py-6 text-center text-gray-500">No bookings yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
